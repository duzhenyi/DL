...
layui.config({
    ...
    oc = {
        ...
        init: function () {
            os.tableLoading();
            os.ajax('Sys/cloudfiles/list', 'POST' ,vm.parm, function (res) {
                if (res.code === 200) {
        ...
        initLocal: function () {
            ...
            os.ajax('Sys/Img/GetList', 'POST' ,vm.localParm,  function (res) {
        ...
        delLocalImage: function (m) {
            os.ajax('Sys/Img/Delete', 'POST' ,{ filename: m.name },   function (res) {
                if (res.statusCode === 200) {
                    vm.list = [];
                    oc.initLocal();
                }
        ...
        //云删除图片
        delImage: function (m) {
            os.ajax('api/cloudfiles/delete', 'POST' , { filename: m.name }, function (res) {
                if (res.code === 200) {
                    vm.list = [];
                    oc.init();
                }
        ...
        //初始化左侧图片分类
        initType: function () {
            os.ajax('Sys/ImgType/GetList', 'POST' , { types: vm.tabActive },  function (res) {
                if (res.statusCode === 200) {
        ...
        //删除图片分类
        delType: function (m) {
            os.ajax('Sys/ImgType/Delete', 'POST' , { parm: m.ID }, function (res) {
                if (res.statusCode === 200) {
                    os.success('删除成功~');
                    oc.initType();
                }
    ...
    form.on('submit(submit)', function (data) {
        ...
        if (vm.typeModel.ID !== '') {
            url = 'Sys/ImgType/Modify';
        }
        //os.log(vm.typeModel);
        os.ajax(url, 'POST',vm.typeModel, function (res) {
            if (res.statusCode === 200) {
                vm.closeType();
                oc.initType();
                os.success('分类保存成功~');
    ...
    form.on('select(ParentId)', function (data) {
        vm.typeModel.ParentId = data.value;
    });
});
